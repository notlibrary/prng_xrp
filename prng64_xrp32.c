...
unsigned char XRP32_TABLE_ID[TABLE_SIZE_BYTES] = {
       92,  6, 85,150, 36, 23,112,164,135,207,169,  5, 26, 64,165,219, //  1
...
static uint64_t
pearson32(uint64_t in[TOTAL_PARAMS])
{
	size_t i;  
	uint64_t h=0;
...
static void
shuffle8bytes(uint64_t a, uint64_t b)
{
...
seed_xrp32(uint64_t seed)
{
   ...
   shuffle8bytes(seed,rotl64(seed,32) );
   for(i = 0; i < (WORDS_IN_TABLE + seed % WORDS_IN_TABLE); ++i) {
		shuffle8bytes(prng64_xrp32(), prng64_xrp32());
   }
   return;
}
