...
unsigned char XRP64_TABLE_ID[TABLE_SIZE_BYTES] = {
       92,  6, 85,150, 36, 23,112,164,135,207,169,  5, 26, 64,165,219, //  1
...
static void
shuffle4bytes(uint32_t a, uint32_t b)
{
...
seed_xrp64(uint32_t seed)
{
   ...
   shuffle4bytes(seed,(rotl32(seed,16)) );
   for(i = 0; i < (WORDS_IN_TABLE + seed % WORDS_IN_TABLE); ++i) {
       shuffle4bytes(prng32_xrp64(), prng32_xrp64());
   }
   return;
}
